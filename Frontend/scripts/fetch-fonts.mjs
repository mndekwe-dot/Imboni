...
async function main() {
    ...
    // The font's own fingerprint, so the test can catch a manifest committed
    // without the font it describes. That happened: nine icons (the library's
    // barcode_scanner among them) were in the manifest but not the woff2, and
    // the scan button rendered as "bar<>_scanner".
    const fontSha256 = createHash('sha256')
        .update(readFileSync(join(OUT, 'material-symbols-subset.woff2')))
        .digest('hex')
    writeFileSync(
        join(ROOT, 'scripts', 'icons.manifest.json'),
        JSON.stringify({ note: 'Generated by scripts/fetch-fonts.mjs — run `npm run fonts` after adding an icon.', fontSha256, icons }, null, 2) + '\n',
    )
